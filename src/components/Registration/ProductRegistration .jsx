...
const userID= UserDetails.user.id;
  ...
  const validationSchema = Yup.object().shape({
    ...
    mrp: Yup.number().min(0, "MRP must be a positive number").required("MRP is required!"),
    rate: Yup.number().min(0, "Rate must be a positive number").required("Rate is required!"),
    productcompany: Yup.string().test(
      "is-not-empty",
              ...
              {!successful && (
                <div>
                    ...
                    name="productcompany" className="form-control"
                      onChange={(e) => {
                        ...
                        companyDeatils.map((companyDeatil) => (
                          (companyDeatil.name === e.target.value) ? setGstpercentages(companyDeatil.gstPercentage) : null
                        ))
                  ...
                  <div className="form-group">
                    ...
                    >{isEdit?<option  value={productDetailsByID?productDetailsByID.vatp:''}>{productDetailsByID.vatp}</option>:<option>--Please Select---</option>}
                      {gstpercentages && gstpercentages.map((gstpercentage, index) => (
                  ...
                  <div className="form-group">
                    ...
                    <Field name="unitesper" type="text" className="form-control" />
                    <ErrorMessage
                      name="unitesper"
                      component="div"
                      className="alert alert-danger"
                  ...
                  <div className="form-group">
                    ...
                    <Field name="is_sp" as="select" className="form-control"
                      onChange={(e) => {
                        ...
                        (val) === 'true' ? console.log("e.target.value is true", e.target.value) : console.log("e.target.value is false", e.target.value);
                        (val) === 'true' ? dispatch(ezshowField()) : dispatch(ezhideField())
                      }

                      }
                    >
                      {isEdit?<option  value={productDetailsByID?productDetailsByID.is_sp:''}>{productDetailsByID.is_sp}</option>:<option>--Please Select---</option>}
                  ...
                  {isenable && (<div className="form-group">
                    <label htmlFor="rel_prod">Related Product</label>
                    <Field as="select" name="rel_prod" className="form-control"
                      disabled={!values.is_sp}
                    > {isEdit?<option  value={productDetailsByID?productDetailsByID.rel_prod:''}>{productDetailsByID.rel_prod}</option>:<option>--Please Select---</option>}
                      {values.is_sp && productDetails && productDetails.map((productDetail, index) => (
