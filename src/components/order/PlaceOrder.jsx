...
import {
  saveBillingDetails,
  getInvoiceDetails,
  updateBillingDetails,saveOrderDetails
...
const PlaceOrder = () => {
  ...
  const handleRegister = async (formValue) => {
    ...
    for (const item of items) {
      ...
      const Orderitem = {
        customer: getIdByName(customerNames, formValue.name),
        product: isEdit?getPidByName(productDetails,item.pname):product.id,
        quantity: item.noofunites,
        orderTakenBy:formValue.userID,
      };

      updatedBillingDetails.push(Orderitem);
    }
